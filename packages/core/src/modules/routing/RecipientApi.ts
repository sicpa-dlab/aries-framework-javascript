import type { V2MediationRequestMessage } from '../../../build'
...
export class RecipientApi {
  ...
  public async pickupMessagesV3(mediatorConnection: ConnectionRecord) {
    ...
    const message = createOutboundDIDCommV2Message(deliveryRequestMessage)
  ...
  public async createRequest(connection: ConnectionRecord): Promise<{
    ...
    if (connection.isDIDCommV1Connection) {
      ...
      const outboundMessage = createOutboundDIDCommV1Message(connection, message)
      return { mediationRecord, message, outboundMessage }
    } else {
