...
import type { DIDCommV2Message, InboundMessageContext, Logger, Transports } from '@aries-framework/core'
import type { WitnessTableMessage } from '@aries-framework/gossip'
...
import { WitnessTableQueryMessage, InjectionSymbols } from '@aries-framework/gossip'
...
export class ValueTransferService {
  ...
  protected config: ValueTransferConfig
  ...
  public constructor(
    ...
    this.logger = agentConfig.logger.createContextLogger('VTP-Service')
    this.config = config
  ...
  public async requestWitnessTable(witnessId?: string): Promise<void> {
    const witness = witnessId || this.config.witnessDid
